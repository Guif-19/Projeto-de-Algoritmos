...
#include <time.h>      // Para inicializar a semente do gerador rand()

// Função para criar diretórios (compatível com Windows e Linux/macOS)
void criar_diretorios_random() {
...
int* gerar_random(int n, int algoritmo) {
    const char *algoritmos[] = {
        "insertion sort", "bubble sort", "shell sort", 
        "selection sort", "merge sort", "quick sort", "heap sort"
    };
    ...
    snprintf(arq_entrada, sizeof(arq_entrada), "%s/arquivos de entrada/random/%d_random.txt", algoritmos[algoritmo - 1], n);
    snprintf(arq_saida, sizeof(arq_saida), "%s/arquivos de saída/random/%d_ordenado.txt", algoritmos[algoritmo - 1], n);
    snprintf(arq_tempo, sizeof(arq_tempo), "%s/arquivos de tempo/random/tempo_random_%d.txt", algoritmos[algoritmo - 1], n);

    // Gera números aleatórios e salva no arquivo de entrada
    ...
    for (int i = 0; i < n; i++) {
        vet[i] = rand() % 100000;  // Gera um número aleatório entre 0 e 99.999
        fprintf(entrada, "%d\n", vet[i]);
    }
    fclose(entrada);
    ...
    printf("Tempo de execução para ordenação aleatória com %d elementos usando %s: %.5lf segundos\n", 
